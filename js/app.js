...
var nextTurn = function() {
    ...
    playerDataRef.update({
        turn: player[myNumber].turn
    })

    if (myNumber < 4) {
        player[myNumber + 1].turn = true;
        playerArrayRef.child(myNumber + 1).update({
            turn: true
        })
    }
    if (myNumber == 4) {
...
function playGame(myPlayerNumber, userId, justJoinedGame, gameRef) {
    ...
    if (justJoinedGame) {
        ...
        playerDataRef.set({
            userId: userId,
            name: player[myPlayerNumber].name,
            bank: player[myPlayerNumber].bank,
            bet: player[myPlayerNumber].bet,
            busted: player[myPlayerNumber].busted,
            blackjack: player[myPlayerNumber].blackjack,
            push: player[myPlayerNumber].push,
            won: player[myPlayerNumber].won,
            lost: player[myPlayerNumber].lost,
            turn: player[myPlayerNumber].turn
        });
    }
